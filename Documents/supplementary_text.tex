...
\begin{table}
  \begin{center}
    \begin{tabularx}{\textwidth}{lXlll}
      ...
      \hline
      $\mu$ & Background death rate & Country specific, see Table S2
      & \cite{World_Development_Indicators2013-ee} \\
      $B$ & Recruitment rate & Country specific, see Table S2
...
\cite{Currie2009-yz}.  The disengagement from treatment moves people
...
infection by the efficacy factor $\rho$.  Without vaccine,
$\zeta = 0$.
...
\begin{equation}
  ...
  \begin{split}
    ...
    N &= S + R +  A + U + D + T + V,
    \\
    \beta_x &= \eta \left[1 - (1 - \tau_x)^n\right]
    \text{ for $x \in \{A, U, T\}$},
    \\
    \tau_T &= \varepsilon \tau_U,
  \end{split}
\end{equation}
where $\eta$ is the transmission rate (\autoref{model_fitting}).

\comment{Describe transmissibilities.}

\comment{Initial conditions.}
The number of people aged 15--49 years \cite{The_World_Bank2016-fd}
and the proportion diagnosed, treated, and with viral suppression
(\autoref{data_sources}) were used to initialize the model for year
2015.  Due to a lack of data on acute infections, and since the acute
phase is so much shorter than the other stages, we assumed that
initially there were no acute infections.
...
\begin{equation}
  ...
  \end{cases}
\end{equation}
with $\chi = 0.001$.  The linear segment connecting $H = 0$ and
$H = 1$ was used to avoid difficulties in computing numerical
solutions that occur when using a discontinuous function.  This
function allows new diagnoses ($\alpha(t) > 0$) when the current
diagnosis level is below the target (i.e., when $p_D^*(t) > p_D(t)$),
and stops new diagnoses ($\alpha(t) = 0$) when the current diagnosis
level is at or above target.  We took
$\alpha_{\max} = 1\;\text{y$^{-1}$}$.
...
suppression seems to depend on the time on treatment (parametrized by
our rate $\gamma$), while relapse rates are difficult to estimate.
...
\begin{equation}
  ...
  \zeta(t) = \zeta_{\max} H\!\left(p_R^*(t) - p_R(t)\right),
\end{equation}
...
then stay constant for the remainder of the time period.  95--95--95
goal is implemented subsequently after meeting 90--90--90 by 2020 by
increasing $p_D$, $p_T$, and $p_V$ to 95--95--95 levels by 2030
linearly.
