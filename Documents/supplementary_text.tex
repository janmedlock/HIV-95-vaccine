...
transmission rate $\eta$ was estimated from each country's
longitudinal HIV prevalence and incidence (\autoref{model_fitting}).
...
15--49 due to data availability, model projections allowed for
analysis of infected people aging beyond age 50.  Due to a lack of
data on acute infections, and since the acute phase is so much shorter
than the other stages, we assumed that initially there were no acute
infections.  Due to a lack of comprehensive data on PLAIDS in each
...
behavior while still allowing for the use of less-recent data to
